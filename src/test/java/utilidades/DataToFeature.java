	        ...
	        try (BufferedReader buffReader = new BufferedReader(
	            ...
	            while ((data = buffReader.readLine()) != null) {
	                ...
	                if (data.trim().contains("##@externaldata")) {
	                    ...
	                    excelFilePath =obtenerAmbientePrueba(excelFilePath);
...
//	                    fileData.add(columnHeader + "|");
	 ...
	 /**Este metodo le agrega al path de la data el ambiente del que debe tomar la data
	  * 
	  * @param data el path 
	  * @return el path mas la carpeta de ambiente (Desarrollo Calidad)
	  */
	public static String obtenerAmbientePrueba(String data) {
		String ambiente = "";
		if (data.contains("AMBIENTE")) {
			Properties p = new Properties(System.getProperties());

			ambiente = p.getProperty("AmbientePrueba");
			if (ambiente != null) {
				data = data.replace("AMBIENTE", ambiente);
			}
		}
		return data;
	}
